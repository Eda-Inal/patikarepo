...
//   all.addEventListener("click", abc)



  menu.forEach(function(currentValue, index){
    ...
    let japan = document.querySelector("#Japan");
    let china = document.querySelector("#China");
    korea.addEventListener("click", abc)
...
div.className = ("col-lg-6 menu-items")
    ...
    currentValue.id == index + 1 ?  div.innerHTML =  `
 
    <img src=${currentValue .img} alt=${currentValue .title} class="photo">
      <div class="menu-info">
        <div class="menu-title">
          <h4>${currentValue .title}</h4>
          <h4 class="price">${currentValue .price}</h4>
        </div>
        <div class="menu-text">
        ${currentValue .desc}
        </div>
    ...
    function abc(){
       menu.filter(item => item.category == "Korea"
